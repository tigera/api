...
$(REPO_ROOT)/queryserver/.image.created-$(ARCH): \
    $(shell $(REPO_ROOT)/hack/image-exists $(REPO_ROOT)/queryserver/.image.created-$(ARCH)) \
    $(call local-deps-go-files,queryserver)
	rm -f $@
	$(MAKE) -C $(REPO_ROOT)/queryserver image
	echo "queryserver:latest-$(ARCH)" > $@

$(REPO_ROOT)/prometheus-service/.image.created-$(ARCH): \
    $(shell $(REPO_ROOT)/hack/image-exists $(REPO_ROOT)/prometheus-service/.image.created-$(ARCH)) \
    $(call local-deps-go-files,prometheus-service)
	rm -f $@
	$(MAKE) -C $(REPO_ROOT)/prometheus-service image
	echo "prometheus-service:latest-$(ARCH)" > $@
...
$(REPO_ROOT)/policy-recommendation/.image.created-$(ARCH): \
    $(shell $(REPO_ROOT)/hack/image-exists $(REPO_ROOT)/policy-recommendation/.image.created-$(ARCH)) \
    $(call local-deps-go-files,policy-recommendation)
	rm -f $@
	$(MAKE) -C $(REPO_ROOT)/policy-recommendation image
	echo "policy-recommendation:latest-$(ARCH)" > $@

$(REPO_ROOT)/voltron/.image.created-$(ARCH): \
    $(shell $(REPO_ROOT)/hack/image-exists $(REPO_ROOT)/voltron/.image.created-$(ARCH)) \
    $(call local-deps-go-files,voltron)
	rm -f $@
	$(MAKE) -C $(REPO_ROOT)/voltron image
	echo "voltron:latest-$(ARCH)" > $@

$(REPO_ROOT)/ui-apis/.image.created-$(ARCH): \
    $(shell $(REPO_ROOT)/hack/image-exists $(REPO_ROOT)/ui-apis/.image.created-$(ARCH)) \
    $(call local-deps-go-files,ui-apis)
	rm -f $@
	$(MAKE) -C $(REPO_ROOT)/ui-apis image
	echo "ui-apis:latest-$(ARCH)" > $@

$(REPO_ROOT)/es-gateway/.image.created-$(ARCH): \
    $(shell $(REPO_ROOT)/hack/image-exists $(REPO_ROOT)/es-gateway/.image.created-$(ARCH)) \
    $(call local-deps-go-files,es-gateway)
	rm -f $@
	$(MAKE) -C $(REPO_ROOT)/es-gateway image
	echo "es-gateway:latest-$(ARCH)" > $@

$(REPO_ROOT)/linseed/.image.created-$(ARCH): \
    $(shell $(REPO_ROOT)/hack/image-exists $(REPO_ROOT)/linseed/.image.created-$(ARCH)) \
    $(call local-deps-go-files,linseed)
	rm -f $@
	$(MAKE) -C $(REPO_ROOT)/linseed image
	echo "linseed:latest-$(ARCH)" > $@
...
$(REPO_ROOT)/webhooks-processor/.image.created-$(ARCH): \
    $(shell $(REPO_ROOT)/hack/image-exists $(REPO_ROOT)/webhooks-processor/.image.created-$(ARCH)) \
    $(call local-deps-go-files,webhooks-processor)
	rm -f $@
	$(MAKE) -C $(REPO_ROOT)/webhooks-processor image
	echo "webhooks-processor:latest-$(ARCH)" > $@

$(REPO_ROOT)/elasticsearch-metrics/.image.created-$(ARCH): \
    $(shell $(REPO_ROOT)/hack/image-exists $(REPO_ROOT)/elasticsearch-metrics/.image.created-$(ARCH)) \
    $(call local-deps-go-files,elasticsearch-metrics)
	rm -f $@
	$(MAKE) -C $(REPO_ROOT)/elasticsearch-metrics image
	echo "elasticsearch-metrics:latest-$(ARCH)" > $@
