...
# The registry when running CI/CD pipelines.
...
release-retag-third-party-base-images-%: var-require-one-of-CONFIRM-DRYRUN var-require-all-RELEASE_BRANCH bin/crane
...
define build_cgo_binary
	$(DOCKER_GO_BUILD_RECURSIVE) \
		sh -c '$(GIT_CONFIG_SSH) mkdir -p $(dir $(2)) && CGO_ENABLED=1 $(if $(CROSS_CC),CC="$(CROSS_CC)") CGO_CFLAGS=$(CGO_CFLAGS) CGO_LDFLAGS=$(CGO_LDFLAGS) go build -o $(2) $(if $(BUILD_TAGS),-tags $(BUILD_TAGS)) -v -buildvcs=false -ldflags "$(LDFLAGS) -s -w" $(1)'
endef

# For binaries that do not require cgo.
define build_binary
	$(DOCKER_GO_BUILD_RECURSIVE) \
		sh -c '$(GIT_CONFIG_SSH) mkdir -p $(dir $(2)) && CGO_ENABLED=0 go build -o $(2) $(if $(BUILD_TAGS),-tags $(BUILD_TAGS)) -v -buildvcs=false -ldflags "$(LDFLAGS) -s -w" $(1)'
endef

define build_binary_dir
	$(DOCKER_GO_BUILD_RECURSIVE) \
		sh -c '$(GIT_CONFIG_SSH) mkdir -p $(1)/$(dir $(3)) && CGO_ENABLED=0 go build -C $(1) -o $(3) $(if $(BUILD_TAGS),-tags $(BUILD_TAGS)) -v -buildvcs=false -ldflags "$(LDFLAGS) -s -w" $(2)'
endef
...
# Spec templates may use @VERSION@ (the full package version) and
# @UPSTREAM_VERSION@ (the raw component version $(3), e.g. for %autosetup -n on an
# unrenamed upstream tarball). Components that don't reference these are unaffected.
define host_native_rpm_build
	$(eval version := $(shell $(REPO_ROOT)/hack/generate-package-version.sh $(REPO_ROOT) rpm "" $(3)))

	sed -e 's/@VERSION@/$(version)/g' \
	    -e 's/@UPSTREAM_VERSION@/$(3)/g' \
	    rhel/$(2).spec.in > rhel/$(2).spec
...
# DEB_PATCHES_DIR is the directory a component stages its quilt patches in,
# relative to the component dir. Defaults to the component dir itself for
# backwards compatibility; components that group patches in a subdirectory
# (e.g. fluent-bit) override it. The series file lists patch basenames, so the
# listing is run from inside DEB_PATCHES_DIR.
DEB_PATCHES_DIR ?= .
...
define host_native_deb_build
	...
	cp $(DEB_PATCHES_DIR)/*.patch package/$(1)/$(2)/debian/patches/ 2>/dev/null || true
	( cd $(DEB_PATCHES_DIR) && ls *.patch 2>/dev/null ) > package/$(1)/$(2)/debian/patches/series || echo -n "" > package/$(1)/$(2)/debian/patches/series
...
# registry specified by REGISTRY. SOURCE_IMAGE lets a component decouple its
# local tag from BUILD_IMAGE - e.g. cmd/calico tags locally as `tigera/calico`
# but publishes as `<REGISTRY>/calico`.
SOURCE_IMAGE ?= $(BUILD_IMAGE)
retag-build-image-arch-with-registry-%: var-require-all-REGISTRY-BUILD_IMAGE-IMAGETAG
	docker tag $(SOURCE_IMAGE):$(LATEST_IMAGE_TAG)-$* $(call filter-registry,$(REGISTRY))$(BUILD_IMAGE):$(IMAGETAG)-$*
	$(if $(filter $*,amd64),\
		docker tag $(SOURCE_IMAGE):$(LATEST_IMAGE_TAG)-$(ARCH) $(REGISTRY)/$(BUILD_IMAGE):$(IMAGETAG),\
		$(NOECHO) $(NOOP)\
	)
...
KIND_CALICO_IMAGES = \
	...
	tigera/envoy-gateway:$(KIND_TEST_BUILD_TAG) \
	tigera/envoy-proxy:$(KIND_TEST_BUILD_TAG) \
	tigera/envoy-ratelimit:$(KIND_TEST_BUILD_TAG) \
	tigera/third-party-cni-plugins:$(KIND_TEST_BUILD_TAG)
...
$(REPO_ROOT)/gateway/.gateway-l7-collector.created-$(ARCH): \
    $(shell $(REPO_ROOT)/hack/image-exists $(REPO_ROOT)/gateway/.gateway-l7-collector.created-$(ARCH)) \
    $(call local-deps-go-files,gateway)
	rm -f $@
	$(MAKE) -C $(REPO_ROOT)/gateway image
...
# WINDOWS_BUILD_ARGS lets a component Makefile pass extra --build-arg flags to
# its Dockerfile.windows build (e.g. fluent-bit passes FLUENT_BIT_VERSION).
