...
# Dataplane for the kind cluster. Currently supports `iptables` (default) and
# `nftables`. Drives both kube-proxy's mode (sed-substituted into the rendered
# kind config) and Calico's `installation.calicoNetwork.linuxDataplane` (passed
# to the helm install in kind-deploy). The two have to match: Felix's
# nftablesMode=Auto follows kube-proxy's mode, so a mismatched cluster ends up
# with stale rules.
DATAPLANE ?= iptables
ifeq ($(filter $(DATAPLANE),iptables nftables),)
  $(error DATAPLANE must be iptables or nftables, got "$(DATAPLANE)")
endif
...
$(REPO_ROOT)/.$(KIND_NAME).created: $(KUBECTL) $(KIND) kind-registry-up
	...
	sed 's/^\(  mode: \)iptables$$/\1$(DATAPLANE)/' $(KIND_CONFIG) > $(KIND_DIR)/.$(KIND_NAME).rendered.config
